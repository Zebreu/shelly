...
use particles::{EmittersCache, EmitterConfig};
...
impl Resources {
    ...
    async fn new() -> Result<Resources, macroquad::prelude::FileError> {
        ...
        let hit_fxses = EmittersCache::new(EmitterConfig {size: 5.6000004, size_randomness: 0.4, ..Default::default()});
        let explosion_fxses = EmittersCache::new(EmitterConfig {size: 15.6000004, size_randomness: 0.4, ..Default::default()});
            //EmittersCache::new(nanoserde::DeJson::deserialize_json(EXPLOSION_FX).unwrap());
        let disarm_fxses =  EmittersCache::new(EmitterConfig {..Default::default()});
...
async fn main() {
    ...
    println!("{}",credentials::NAKAMA_SERVER);
